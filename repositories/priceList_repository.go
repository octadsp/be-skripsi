...
func (r *repository) FindPriceLists(offset, limit int) ([]models.PriceList, error) {
	...
	err := r.db.Offset(offset).Preload("CarClass").Preload("DemageSubCategory").Preload("DemageCategory").Limit(limit).Order("id").Find(&prices).Error // Using Find method
...
func (r *repository) FindAllPriceLists() ([]models.PriceList, error) {
	var prices []models.PriceList
	err := r.db.Order("id").Preload("CarClass").Preload("DemageSubCategory").Preload("DemageCategory").Find(&prices).Error // Using Find method
